...
function setup() {
    ...
    mainCanvas.position(요소위치.캔버스위치);
    
    fill(0, 30);
    업데이트();
    console.log(자소기준점리스트);
    //User Interface
...
function getLetter(letterString) {
    ...
    for (var i = 0; i < 사용자입력문장분할.length; i++){
        ...
        if (사용자입력문장분할[i].length === 1 && 사용자입력문장분할[i][0] === ' ') {
            ...
            console.log(사용자입력문장분할[i].length);
        }
        console.log(사용자입력문장분할[i]);
